...
const Canvas = props => {
    ...
    useEffect(() => {
        ...
        function lineEnd(event) {
            ...
            const image = DContext.getImageData(minX, minY, maxX - minX, maxY - minY);//pX-pY-sW-sH
            console.log(image)

            //reduce size of drawing canvas to the bare minimum size;
            ...
            //mock two images from a random drawing
            const aSpec = {
                width: image.width,
            ...
            //prepare a third imageData object for the final result
            const xWidth = image.width + (Math.max(aSpec.x, bSpec.x) - Math.min(aSpec.x, bSpec.x)) //width plus offsetX
            const xHeight = image.height + (Math.max(aSpec.y, bSpec.y) - Math.min(aSpec.y, bSpec.y))//height plus offsetY
            const xSpec = {
                width: xWidth,
                height: xHeight,
                x: Math.min(aSpec.x, bSpec.x),//top left corner X coordinate
                y: Math.min(aSpec.y, bSpec.y)//top left corner Y coordinate
            }

            console.log(xSpec)

            //create blank imageData object and fill with white pixels
            const xImage = context.createImageData(xSpec.width, xSpec.height)
            /*
            for (let i = 0; i < xImage.data.length; i++) {
                xImage.data[i] = 255
            }
*/
            //assuming aSpec is at point zero

            for (let xx = 3; xx <= aSpec.width * 4; xx += 4) {//the fourth position is the alpha value
                //the image data has four values for each pixel, hence the length of the image is four times the width
                //run through the x axis of aSpec
                for (let yy = 0; yy <= aSpec.height; yy++) {//run through the y axis of aSpec
                    if (aSpec.data[xx + (yy * aSpec.width * 4)] !== 0) {//test transparency value
                        xImage.data[xx + (yy * xImage.width * 4)] = aSpec.data[xx + (yy * aSpec.width * 4)];
                        xImage.data[xx + (yy * xImage.width * 4) - 1] = aSpec.data[xx + (yy * aSpec.width * 4) - 1];
                        xImage.data[xx + (yy * xImage.width * 4) - 2] = aSpec.data[xx + (yy * aSpec.width * 4) - 2];
                        xImage.data[xx + (yy * xImage.width * 4) - 3] = aSpec.data[xx + (yy * aSpec.width * 4) - 3];
                    }
            ...
            for (let xx = 3; xx <= bSpec.width * 4; xx += 4) {
                for (let yy = 0; yy <= bSpec.height; yy++) {

                    if (bSpec.data[xx + (yy * bSpec.width * 4)] !== 0) {

                        xImage.data[xx + 100 + (yy + 100) * xImage.width * 4] = bSpec.data[xx + (yy * bSpec.width * 4)];
                        xImage.data[xx + 100 + (yy + 100) * xImage.width * 4 - 1] = bSpec.data[xx + (yy * bSpec.width * 4) - 1];
                        xImage.data[xx + 100 + (yy + 100) * xImage.width * 4 - 2] = bSpec.data[xx + (yy * bSpec.width * 4) - 2];
                        xImage.data[xx + 100 + (yy + 100) * xImage.width * 4 - 3] = bSpec.data[xx + (yy * bSpec.width * 4) - 3];
                    }
